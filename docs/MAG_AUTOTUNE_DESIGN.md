...
> **改訂(2026-08-03 — motionソースのアーム相対整列・ヨーアンカーの
> 地上ローリング再ラッチ化)**:
> - **(a) motionソースのワイヤ変換を mocap と同一の整列へ変更**: 当初実装の
>   motionソース送信値は heading×wire_sign(パネル較正 yaw_offset の逆写像)
>   経路で、P0-1 のアーム相対アンカーを通らないため、18:20故障モードと同型の
>   定数オフセット基準が motion 切替時に**構造的に再発**する(7/31実ログ検証:
>   旧経路の送信値は機体制御フレームに対し −95.5°±2.3° / −62.7°±6.7° ⇒
>   EKF2 の30°ゲート内 **0%** ⇒ 全棄却→融合停止ラッチ)。本改修で旧経路を
>   廃止し、推定 ψ̂(PoC規約 ψ̂≈−yaw_true)を −ψ̂ で yaw_true 系へ戻した上で
>   mocapソースと**同一の整列規則**を適用する: `align=arm`(既定)は
>   `wrap(−ψ̂ + anchor)`(アンカー未ラッチ時は bit3=0)、`align=absolute` は
>   `wrap(−ψ̂)` — ソース切替でファームが見るフレームが変わらない。合わせて
>   **ログ列 `motion_yaw_rad` の規約も旧ワイヤ規約からこの整列適用後の
>   送信規約**(= `yaw_ref_sent_rad` と同一フレーム)へ変更
>   (LOG_STRUCTURE.md §16)。
> - **(b) ヨーアンカーの地上ローリング再ラッチ化**: 当初実装の地上暫定
>   ラッチは1回きりで、前フライト/前セッションの stale アンカーが地上待機中の
>   機体ヨードリフト(EKF1で最大 −90°/min 実測)から乖離し、アーム再ラッチで
>   基準が段差ジャンプ → EKF2 が段差を b_g へ誤吸収して恒常イノベーション
>   プラトー化する(7/31 22:38実測: 段差 −8.46° → 飛行RMS 2.03°。21:15でも
>   +8.4°段差)。本改修で**地上では毎tick 0.5s窓の円周中央値で再ラッチし続け
>   (ローリング)、アーム遷移で凍結**(飛行中は1フライト1回のまま)に変更 —
>   地上で基準を機体フレームへ追従させ続ければアーム時の段差は構造的に
>   生じない。「地上は未ラッチ扱い(bit3=0)」案は地上融合を止め、プリ
>   フライト fused チェックと est_mode=2 の離陸インターロックを壊すため
>   不採用。ログ・meta.json への記録は初回ラッチとアーム遷移のみ(値の更新は
>   毎tick反映)。
>   **整合ガード(スプレッドガード)**: 地上ローリングは窓内 diff の円周
>   中央値からの最大絶対偏差(スプレッド)が 10° 未満のときのみ再ラッチする。
>   超過中は直前アンカーを凍結維持し(2s 継続で警告・10s クールダウン)、
>   鏡映フレーム(yaw_true 符号誤り)の手回しでは diff が 2ψ̇ で流れて凍結
>   → イノベーション増大がプリフライト・インターロック(P1-2)に見える —
>   ローリング化で失われかけた「アーム前フレーム誤り検出力」の回復措置。
>   正常フレームの手回し(diff 一定)と緩慢な地上ドリフト(1.5°/s 級 →
>   0.5s 窓スプレッド <1°)は追従が継続する。アーム遷移ラッチは無条件
>   (中央値がロバスト)。
> - **(c) motionソースの未対応マッピングガード**: `machine_wire_y_sign` が
>   None(軸入替等の未対応マッピング)の間は motion 送信値を生成しない
>   (bit3=0)。PoC 規約 ψ̂≈−yaw_true は対応マッピングでのみ実証されており、
>   未対応フレームでは LS モデル自体が未検証のため(XY 制御の無効化と同じ
>   判定基準)。
>
> 整列規則・ラッチ規則の最終仕様は**本コミットの実装(pc_server/core/
> session.py `_update_motion_yaw` / `_update_yaw_anchor`)が正**。
...
> **改訂(2026-08-03 — motion_yaw v2: derotation+速度回帰項+lag丸め修正)**:
> モデルを `a_W(t) = e^{jψ(t)}·u_B(t) + λ·v_W(t) + noise` へ拡張:
> - **derotation**: θ=∫r dt(テレメトリのヨーレート r。25Hzサンプルホールドで
>   可)で Ũ=e^{j·s·θ}·U と回すと窓内定数 α=e^{jψ_ref} の線形LSになり、
>   ψ̂_now=arg(α)+s·θ_now — 回頭中の窓平均ラグ(≈窓長/2、実測 2〜6.7s)を
>   除去する。
>   符号 s=−1 は実ログ 20260731_211535(±100°回頭ペア)のリプレイで確定:
>   全飛行RMS 26.1°(OFF)→ **12.0°**(s=+1 だと 45.7° に悪化)。
> - **速度回帰項**: λ(複素)は速度比例の空力項(PoC実測 k_re≈0.33 /
>   k_im≈0.39 [1/s])を吸収 — 純ホバでも残る +25°級ブロックバイアスの主因
>   対策。2×2複素正規方程式で α, λ を解き、U と V がほぼ共線(純円運動等)は
>   条件数ガード(det < 0.02·S_uu·S_vv)で1回帰子 α=S_au/S_uu へフォール
>   バック。estimate() に診断キー `lambda_re`/`lambda_im`/`vel_term_active`
>   を追加。
> - **lag丸め修正**: delay_s の tick 換算を half-up 化(0.09s×50Hz=4.5 →
>   5 tick。旧 round() は銀行家丸めで 4 tick=80ms に落ちる回帰があった)。
>   合わせて遅延の因果ラベルを訂正 — 遅い側はフィルタ済 MoCap 位置経路
>   (A系列)であり「テレメトリ遅延」は誤り。
>
> config 新キー: `motion_yaw.derotate` / `motion_yaw.velocity_term`
> (いずれも既定 true。control.json の motion_yaw 節で上書き可)。
> 数式・符号・ガード定数の最終仕様は**本コミットの実装
> (pc_server/core/motion_yaw.py)が正**。
